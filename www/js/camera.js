  ...
  $('#Parks').on('click', '#delete', function(event) {
	...
	ImgData.set("objectId", data_objectId)
      ...
      .then(function(result){
			ons.notification.alert({title:"削除",message: '写真を削除しました。'});
      		//photopage();
      		})
...
function onFormSend() {
    ...
    if (src == ""){
	     	$('.blue_btn').show();
			$('.blue_btn2').hide();
		ons.notification.alert({title:'エラー',message:"写真を挿入してください。"});
    			return;
    }
    ...
    if (name == ""){
	     	$('.blue_btn').show();
			$('.blue_btn2').hide();
		ons.notification.alert({title:'エラー',message:"写真のタイトルを入力してください。"});
    			return;
    }
    var EnjiClass=[];
		for(for_c = 0;for_c < class_datalist.length ;for_c++){
    ...
    NCMB.Push.send({
      ...
      searchCondition: {"albume":"受信する"},
							...
							Phototouroku_page();
					    ons.notification.alert({title:'アップロード',message:"アップロード成功"});
						     	$('.blue_btn').show();
								$('.blue_btn2').hide();
							})
							.catch(function(err){
...
function oshirase_onFormSend() {
    ...
    var EnjiClass=[];
		...
		//alert("全体クラス"+class_datalist.length+"選択クラス"+EnjiClass.length)
	var push_search="";
	if(class_datalist.length == EnjiClass.length){
		push_search = "all";
	}
	EnjiClass.push("管理者");
	  ...
	  var countup = function(){
	    ...
	    if(count > 1){　
	      clearInterval(id);　//idをclearIntervalで指定している
			    ref.close();
		ons.notification.alert({title:'送信',message:"送信完了"});
	    }}, 1000);
	    });
	}else{
	    NCMB.Push.send({
...
function event_tuika() {
	...
	EnjiClass.push("管理者");
					...
					event.set("name", name)
							...
							.save()
							.then(function(event){
								ons.notification.alert({title:'追加',message:"イベントが追加されました"});
    							$('input').val("");
