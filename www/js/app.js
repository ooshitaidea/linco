        var appKey    = "f5677f799279f9bbee77a69812cc81f89c492c072d5e9caa0e6697b43371040f";	//LinCo appKey
        var clientKey = "98e148b00d28a9d6267b9764b0a8784e19d5e2bc4636c0809f79962c756d57cc"; //LinCo clientKey
        ...
        var appname = "LinCo(L307)";
        var mailsave = "http://ideaxidea.net/phpmail/index.php?title=";
        var mode = "ビルド";//debug or ビルド
		var en_point  = ["Ideaideal",new google.maps.LatLng(33.262515,130.296944)];	//イデア
		var class_datalist = ["Cクラス","Bクラス","Aクラス"];
		function homepage() {var ref = window.open('http://ideaideal.jp/we-have-started-the-app-service-named-linco/', '_blank');}
...
function onLoginBtn() {
	...
	if (username == ""){
		ons.notification.alert({title:"エラー",message: '保護者名が入力されていません'});
		$(".load").hide();
		return;
	}else if(password == ""){
		ons.notification.alert({title:"エラー",message: 'パスワードが入力されていません'});
		$(".load").hide();
		return;
	}
    ...
    // ユーザー名とパスワードでログイン
			    ...
			    ncmb.User.login(username, password)
			        .then(function(user) {
					    ...
					    ons.notification.alert({title:'ログイン',message:"ログインしました。"});
            ...
            for (var i = 0; i < results.length; i++) {
              var object = results[i];
				...
				if(check_Name == username){
						ons.notification.alert({title:"エラー",message:check2});
						user_on = "ok";
				}else{
					if(i >= results.length-1 && user_on !== "ok"){
						ons.notification.alert({title:"エラー",message:check1});
					}
				}
  ...
  $('#push_list').on('click', '#delete', function(event) {
	...
	oshirase.set("objectId", data_objectId)
      ...
      .then(function(result){
      		$(".load").hide();
			ons.notification.alert({title:"削除",message: 'お知らせを削除しました。'});
      		//osirase();
      		})
...
function onRegisterBtn2() {
    ...
    user.signUpByAccount()
        .then(function(user) {
            ...
            currentLoginUser = ncmb.User.getCurrentUser();
					    if (checgroup !== "管理者") {
							$('.admin').hide();$('.ippan').show();
						}else{
							$('.ippan').hide();$('.admin').show();
					    };
		    
		//ユーザデータデータベース作成
		var UserData = ncmb.DataStore("UserData");
		var UserData = new UserData();
			UserData.set("user_id", user.objectId);
			UserData.set("user_name", user.userName);
			UserData.set("albume", "受信しない")
			UserData.set("photo_open", user.createDate)
			UserData.set("oshirase_open", user.createDate)
			.save()
			.then(function(user_data){
		    	$.mobile.changePage('#TopPage');
...
function contact_delete(){
  ...
  $('#contact_list').on('click', '#delete', function(event) {
	...
	ContactData.set("objectId", data_objectId)
      .delete()
      .then(function(result){
			ons.notification.alert({title:"削除",message: 'メッセージを削除しました。'});
      		//photopage();
      		})
  ...
  NCMB.initialize(appKey, clientKey);
    ...
    NCMB.Push.send({
	 ...
	 window.scroll(0,$(document).height());
	         ...
	         .catch(function(err){
	            alert(err);
    			ons.notification.alert({title:"エラー",message: 'NCMBからの読み込みができませんでした！'});
	          });
		...
		UserData.equalTo('user_id',objectId).fetch()	//現在のログインユーザーがバス情報の中に入ってないかチェック
		   .then(function(user_data){
				...
				if(albume == "受信する"){
					...
					$('#img_henkou_btn').attr("name","受信しない");
				} else {
					$('#imgpush_now').html("受信しない");
					$('#img_henkou_btn').html("受信する");
...
function onClassHenkouBtn()
{
    ...
    ons.notification.alert({title:"クラス変更",message: "クラスを" + henkouclass + "に変更しました"});
   			 $.mobile.changePage('#TopPage');
			ncmb.User.login(userName, password);
          })
          .catch(function(err){
...
function onBusHenkouBtn()
{
	...
	currentUser.update()
		.then(function(data){
			ons.notification.alert({title:"バス変更",message: "バス情報を" + henkou_bus + "に変更しました"});
			$.mobile.changePage('#TopPage');
			//ncmb.User.login(userName, password);
		})
		.catch(function(err){
			ons.notification.alert({title:"エラー",message: "バス情報の変更に失敗しました" + err});
		});
...
function onimgHenkou() {
	var img_henkou_btn = $("#img_henkou_btn").attr("name");
...
function eventDeleteLink() {
  $('#calendar_li').on('click', '#delete', function(event) {
	...
	event.set("objectId", data_objectId)
      .delete()
      .then(function(result){
			ons.notification.alert({title:"削除",message: '予定を削除しました。'});
			calendar_open()
      		//photopage();
      		})
      .catch(function(err){alert(err+"で失敗しました。");});
...
var currentUser = ncmb.User.getCurrentUser();
	if (currentUser) {
		...
		ons.notification.alert({title:"エラー",message: "ユーザー情報が読み込めません"});
	}
}
...
function onReady() {
   ...
   //onsenui読み込み
    ...
    //ons.notification.alert({title:"エラー",message: 'このアプリはログインする必要があります。'});
	    $.mobile.changePage('#LoginPage');
	}
